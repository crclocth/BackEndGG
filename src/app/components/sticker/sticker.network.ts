...
router.get("/all", async function(req: Request, res: Response){
    ...
    try {
        const result: Sticker[] = await stickerController.getSticker();
...
router.get("/:id", async function(req: Request, res: Response){
    ...
    try {
        const result: Sticker | null = await stickerController.getStickerById(id);
        responseModule.success(req,res,result);
...
router.post("/add", async function(req: Request, res: Response){
    ...
    const body: Sticker = req.body;
    try {
        const result: Sticker = await stickerController.addSticker(body);
...
router.delete("/delete", async function (req: Request, res: Response) {
    try {
      const ver = await stickerController.deleteSticker(req.body._id);
      if (ver != null) {
...
router.patch("/:id", async function(req: Request, res: Response){
    const { id } = req.params;
    try {
        const modPaciente = await stickerController.patchSticker(id, req.body);
